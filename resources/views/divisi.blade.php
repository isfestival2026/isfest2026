<x-layout.app title="Divisi — ISFEST 2026">
    ...
    @php
    // BENCANA TERATASI: Idealnya array data ini lo pindahkan ke DivisiController.php
    // Lalu passing ke view menggunakan compact('divisi', 'sovereign', 'otherDivisi')
    
    // URL Backblaze Base (Contoh persiapan)
    $backblazeBaseUrl = env('BACKBLAZE_URL', '/logo%20Divisi'); 
    ...
    $divisi = [
        [
            ...
            'label'  => 'BPH',
            'folder' => 'Sovereign',
            // Siap untuk URL Backblaze spesifik, atau pakai fallback default
            'logo_url' => $backblazeBaseUrl . '/Sovereign.png',
            'members' => [
...
[
    ...
    <div class="bg-[#0a101d] text-slate-200 min-h-screen pb-16">
        
        {{-- Komponen Hero Sovereign --}}
        <x-divisi.hero :data="$sovereign" />
        ...
        <div class="max-w-6xl mx-auto px-6 pt-14 pb-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
                @foreach($otherDivisi as $index => $d)
                    {{-- Index ditambah 1 karena array otherDivisi dimulai dari elemen ke-2 --}}
                    <x-divisi.card :data="$d" :index="$index + 1" />
                @endforeach
            </div>
        </div>

        {{-- Komponen Modal JS --}}
        <x-divisi.modal :allData="$divisi" />
