<x-layout.app title="Divisi — ISFEST 2026">
    ...
    // Mengambil dan menghasilkan Temporary URL dari Backblaze (Private Bucket)
    foreach ($divisi as &$d) {
        // Asumsi folder di dalam bucket Backblaze lo adalah 'logo-divisi'
        $pathFile = 'logo-divisi/' . $d['name'] . '.png';
        
        try {
            // Membuat URL sementara yang valid selama 60 menit (bebas lo sesuaikan)
            $d['logo_url'] = Storage::disk('s3')->temporaryUrl($pathFile, now()->addMinutes(60));
        } catch (\Exception $e) {
            // Fallback aman: Kalau misal konfigurasi S3 lo di .env belum selesai, 
            // dia akan otomatis nyari ke folder public/logo Divisi lokal di MacBook lo dulu.
